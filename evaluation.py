...
def map_ref_pred(ref_boxes, pred_boxes, mapping, reverse_mapping, all_confs, epsilon=0.5):
    '''
...
def process_mappings(mapping, reverse_mapping, all_confs):
    ...
    for k in mapping.keys():
        ...
        if len(matches) == 0:
            ...
            FN_thresh = np.max(
                [match[2] for match in matches])  # As soon as the first match appears, it is no longer a FN
    ...
    for k in reverse_mapping.keys():
        ...
        if len(matches) != 0:
            for match in matches:
                ...
                ordering = list(np.argsort([-rm[1] for rm in ref_matches]))  # Sort the matches by descending IoU
                curr_elem = [rm[0] for rm in ref_matches].index(k)
                rank = ordering.index(curr_elem)  # Where is the current element in the ordering?
                if rank == 0:
                    FP_thresh = -1.0  # If this is the best match for any ref box, then it's never a FP
                    break
                else:
                    all_match_confs = [ref_matches[ordering[ridx]][2] for ridx in np.arange(rank - 1, -1,
                                                                                            -1)]  # Find the confidences of all the matches with higher IoU
                    min_match_conf = np.min(all_match_confs)
                    if min_match_conf > all_confs[
                        k]:  # There is a better match that appears before this one. For this ref box, it's a FP.
                        pass
                    else:
                        FP_thresh = min(FP_thresh,
                                        min_match_conf)  # This match is the first until another one appears later. What is the lowest confidence for which this is still not a FP?
...
def compute_avg_precision(project_code, folder_ref, folder_pred):
    ...
    confidences = confidences[np.argsort(-confidences)]  # Sort the confidences in descending order
    ...
    interp_prec = np.flip(np.maximum.accumulate(np.flip(precisions)))  # Use the interpolated prec-rec curve
    interp_rec = recalls
...
def get_arguments():
    if len(sys.argv) != 4:
        print("Usage: python evaluation.py <project_code> <path_2_reference> <path_2_output_predictions>")
        sys.exit(1)
...
if __name__ == "__main__":
    ...
    if project_type == 'DET':
        score = compute_avg_precision(project_type, path_2_ground_truth, path_2_predictions)
        ...
        score = compute_avg_iou_mask(project_type, path_2_ground_truth, path_2_predictions)
    elif project_type == 'COL' or project_type == 'SUP':
        score = compute_simple_score_images(project_type, path_2_ground_truth, path_2_predictions)
